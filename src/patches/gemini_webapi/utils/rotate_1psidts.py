...
async def rotate_1psidts(cookies: dict, proxy: str | None = None) -> str:
    ...
    `curl_cffi.requests.errors.RequestError`
    ...
    if not (path.is_file() and time.time() - os.path.getmtime(path) <= 60):
        logger.debug("[rotate_1psidts] Rotating __Secure-1PSIDTS cookie...")
        async with AsyncSession(proxy=proxy, impersonate="chrome") as session:
        ...
        logger.debug(f"[rotate_1psidts] Rotation response status={response.status_code}")
