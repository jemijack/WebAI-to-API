...
async def send_request(
    ...
    async with AsyncSession(
        ...
        response = await session.get(Endpoint.INIT.value)
...
async def get_access_token(
    ...
    async with AsyncSession(proxy=proxy, allow_redirects=True, verify=False, impersonate="chrome") as session:
        response = await session.get(Endpoint.GOOGLE.value)
